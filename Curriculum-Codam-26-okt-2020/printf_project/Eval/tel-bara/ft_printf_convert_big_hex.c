...
/*   Updated: 2020/12/04 14:53:37 by tel-bara      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf.h"

static void	ft_printf_big_hex_left_padding(unsigned long long nbr, int numlen,
...
static void	ft_printf_big_hex_right_padding(unsigned long long nbr, int numlen,
t_flags *flags, int *count)
{
	int			right_padding;
	int			prefix;
...
void		ft_printf_convert_big_hex(t_flags *flags, va_list args, int *count)
{
	...
	if (flags->length_modifier_l)
		nbr = (unsigned long)va_arg(args, long int);
	...
	if (!nbr && !flags->precision_value)
		numlen = 0;
	ft_printf_big_hex_left_padding(nbr, numlen, flags, count);
	if (nbr || flags->precision_value)
		ft_putlunbr_base_fd(nbr, "0123456789ABCDEF", 1);
	ft_printf_big_hex_right_padding(nbr, numlen, flags, count);
	*count += numlen;
}
